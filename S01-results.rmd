...
stratified_prevalence <- function(eids_elig, dt_bl, dt_dem) {
  ...
  dt_prev <- data.table(eid = eids_elig) %>%
    ...
    left_join(dt_bl %>% select(eid, stage, kdigo_risk), by = "eid") %>%
    mutate(
      has_ckd = !is.na(stage),
  ...
  out <- rbindlist(list(strat, by_age, by_sex, grand), use.names = TRUE) %>%
    mutate(
      prev_ckd_pct        = round(100 * n_ckd / n_total, 2),
      prev_stage3plus_pct = round(100 * n_stage3plus / n_total, 2)
    ) %>%
    arrange(
...
cat("\n=== Age- and Sex-Stratified Baseline Prevalence: Full Cohort ===\n")
print(prev_full, nrows = Inf)

out_file_full <- "data/outputs/S01-prevalence-baseline-full.csv"
dir.create(dirname(out_file_full), showWarnings = FALSE, recursive = TRUE)
fwrite(prev_full, out_file_full)
upload_to_dx(out_file_full, "ukb-kdsc-staging/outputs/S01-prevalence-baseline-full.csv")
cat(sprintf("Saved: %s\n", out_file_full))
```

### 9.2 GP-Linked Sub-Cohort
```{r}
prev_gp <- stratified_prevalence(eids_eligible_gp, dt_baseline, dt_demog)

cat("\n=== Age- and Sex-Stratified Baseline Prevalence: GP-Linked ===\n")
print(prev_gp, nrows = Inf)

out_file_gp <- "data/outputs/S01-prevalence-baseline-gp.csv"
fwrite(prev_gp, out_file_gp)
upload_to_dx(out_file_gp, "ukb-kdsc-staging/outputs/S01-prevalence-baseline-gp.csv")
cat(sprintf("Saved: %s\n", out_file_gp))
```
