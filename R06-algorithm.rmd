...
dt_eligible_egfr <- dt_egfr_acr %>%
  ...
  filter(date_diff >= 90) %>%
  summarise(n_pairs_90days_apart = n()) %>%
  filter(n_pairs_90days_apart >= 1) %>%
  as.data.table()
...
upload_to_dx(out_file, "kdsc_staging/ckd_timeline.parquet")

out_file <- "data/ckd_baseline.parquet"
write_parquet(dt_baseline, out_file)
upload_to_dx(out_file, "kdsc_staging/ckd_baseline.parquet")

out_file <- "data/ckd_10y.parquet"
write_parquet(dt_10y, out_file)
upload_to_dx(out_file, "kdsc_staging/ckd_10y.parquet")
...
upload_to_dx(out_file, "kdsc_staging/ckd_eligible.parquet")
```
