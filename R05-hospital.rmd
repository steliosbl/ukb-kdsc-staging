...
dt_diagnoses <- fread("../data/common/Hospital Records/diagnoses.csv",
  na.strings = c("", "NA")
)
dt_operations <- fread("../data/common/Hospital Records/operations.csv",
  na.strings = c("", "NA")
)

# Code reference tables
dt_icd10 <- fread("../data/common/Hospital Records/icd10_codes.csv")
dt_opcs4 <- fread("../data/common/Hospital Records/opcs4_codes.csv")

# Self-reported conditions
dt_medical_history <- fread("../data/common/Medical History/medical_history.csv",
  na.strings = c("", "NA")
)

# GP clinical records (for ICD-10 matching via mapped codes)
dt_gp_clinical <- tryCatch(
  fread("../data/common/Primary Care/gp_clinical_records.csv", na.strings = c("", "NA")),
...
upload_to_dx(out_file, "kdsc_staging/ckd_matched_events.parquet")

out_file <- "data/selfreport_kidney.parquet"
write_parquet(dt_selfreport, out_file)
upload_to_dx(out_file, "kdsc_staging/selfreport_kidney.parquet")
