...
run_table_exporter(
  ...
  entity = "participant",
  output_name = "kdsc_staging/demographics_raw",
  dest_folder = "data/raw/demographics"
)
...
run_table_exporter(
  ...
  entity = "participant",
  output_name = "kdsc_staging/biomarkers_raw",
  dest_folder = "data/raw/biomarkers",
...
for (entity_type in unique(gp_fields$entity)) {
  run_table_exporter(
    ...
    entity = entity_type,
    output_name = sprintf("kdsc_staging/gp_%s_raw", entity_type),
    dest_folder = sprintf("data/raw/gp_%s", entity_type),
    instance_type = "mem1_ssd1_v2_x16"
  )
}
...
for (entity_type in c("hesin", "hesin_diag", "hesin_oper")) {
  run_table_exporter(
    ...
    entity = entity_type,
    output_name = sprintf("kdsc_staging/hospital_%s_raw", entity_type),
    dest_folder = sprintf("data/raw/hospital_%s", entity_type),
    instance_type = "mem1_ssd1_v2_x16"
  )
}

# Curate (reproduces data/common/Hospital Records/scripts/curate_data.R)
source("curate/curate_hospital_records.R")
```
...
run_table_exporter(
  ...
  entity = "participant",
  output_name = "kdsc_staging/medical_history_raw",
  dest_folder = "data/raw/medical_history",
  instance_type = "mem1_ssd1_v2_x16"
)
...
## 7. Upload All Extracted Data
```{r}
# Upload all curated common data to DNAnexus
for (dir_name in list.dirs("data/common", recursive = FALSE, full.names = FALSE)) {
  files <- list.files(file.path("data/common", dir_name), pattern = "\\.(csv|txt)$", full.names = TRUE)
  for (f in files) {
    upload_to_dx(f, file.path("common", dir_name, basename(f)))
  }
}
```
